...
cleanly leaves the session in a file unity-sync also looks for. `--session browser` does
the same thing for one run, and the run prints which profile it read.
...
look for. Whatever you point it at, unity-sync works out what the file is by reading it.
